...
const RaceShow = () => {
  ...
  const columns: ColumnsType<IRanking> = [
    ...
    },
    {
      ...
      render: (_, item) => {
        return (
          <Link to={`/driver/show/${item.driver.id}`}>
            {item.driver.name.concat(" (" + item.driver.abbr + ")")}
          </Link>
        );
      },
    },
    ...
    },
    {
      ...
      key: "grid",
      render: (_, item) => {
        return <GridProgression grid={item.grid} position={item.position} />;
      },
    },
  ];
  return (
    <div>
      <Link to="/">Back</Link>
      <Table bordered dataSource={rankings} columns={columns}></Table>
    </div>
  );
};
